...
from . models import Customer,Laptop,Order,Cart
...
class NormalCategoriesView(View):
    def get(self,request):
        normal_category = Laptop.objects.filter(category='NORMAL')  # we are using filter function of queryset, that will filter those data whose category belongs to dog
        return render(request,'core/normal_categories.html',{'normal_category':normal_category})


#--- Class Based View of cat_categories ---
class GamingCategoriesView(View):
    def get(self,request):
        gaming_category = Laptop.objects.filter(category='GAMING')  # we are using filter function of queryset, that will filter those data whose category belongs to dog
        return render(request,'core/gaming_categories.html',{'gaming_category':gaming_category})

class LaptopDetailView(View):
    def get(self,request,id):     # id = It will fetch id of particular Laptop 
        laptop = Laptop.objects.get(pk=id)

        #------ code for caculate percentage -----
        if Laptop.discounted_price !=0:    # fetch discount price of particular Laptop
            percentage = int(((laptop.selling_price-laptop.discounted_price)/laptop.selling_price)*100)
        else:
            percentage = 0
        # ------ code end for caculate percentage ---------
            
        return render(request,'core/laptop_details.html',{'Laptop':laptop,'percentage':percentage})
...
def add_to_cart(request, id):    # This 'id' is coming from 'Laptop.id' which hold the id of current Laptop , which is passing through {% url 'addtocart' Laptop.id %} from Laptop_detail.html 
    if request.user.is_authenticated:
        product = Laptop.objects.get(pk=id) # product variable is holding data of current object which is passed through 'id' from parameter
        user=request.user                # user variable store the current user i.e steveroger
        Cart(user=user,product=product).save()  # In cart model current user i.e steveroger will save in user variable and current Laptop object will be save in product variable
        return redirect('laptopdetails', id)       # finally it will redirect to Laptop_details.html with current object 'id' to display Laptop after adding to the cart
    else:
...
def payment_success(request,selected_address_id):
    ...
    for c in cart:
        Order(user=user,customer=customer_data,Laptop=c.product,quantity=c.quantity).save()
        c.delete()
...
def buynow(request,id):
    laptop = Laptop.objects.get(pk=id)     # cart_items will fetch product of current user, and show product available in the cart of the current user.
    delhivery_charge =2000
    final_price= delhivery_charge + laptop.discounted_price
    
    address = Customer.objects.filter(user=request.user)

    return render(request, 'core/buynow.html', {'final_price':final_price,'address':address,'Laptop':laptop})
...
def buynow_payment(request,id):
    ...
    return render(request, 'core/payment.html', {'final_price':final_price,'address':address,'Laptop':laptop,'paypal':paypal_payment})
...
def buynow_payment_success(request,selected_address_id,id):
    ...
    laptop = Laptop.objects.get(pk=id)
    Order(user=user,customer=customer_data,Laptop=laptop,quantity=1).save()
